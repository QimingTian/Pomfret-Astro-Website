...
test('past scheduled slot is re-placed instead of kept', () => {
  withMockedNow(() => {
    const pastStart = new Date(NOW_MS - 3 * 3600_000).toISOString()
    const row = baseRow({
      id: 'row-past',
      status: 'scheduled',
      plannedStartIso: pastStart,
      filterPlans: [{ filterName: 'H', exposureSeconds: 60, count: 10 }],
    })
    const insight = computeScheduleInsight([row], row.id, moonHighWeatherWindow())
    assert.equal(insight.status, 'scheduled', insight.reasons.join(' | '))
    assert.ok(insight.plannedStartIso, 'expected a new planned start')
    assert.notEqual(insight.plannedStartIso, pastStart)
    assert.ok(Date.parse(insight.plannedStartIso!) >= NOW_MS)
  })
})
