...
} from '@/lib/imaging/observatory-local-time'

export { observatoryLocalCalendarAnchorUtc } from '@/lib/imaging/observatory-local-time'
...
export function getObservatoryTonightWallWindow(now = new Date()): {
  ...
  const startParts = observatoryLocalParts(start, lon)
  const endAnchor = new Date(Date.UTC(startParts.year, startParts.month, startParts.day + 1))
  const end = observatoryWallTimeOnLocalDateUtc(endAnchor, 8, 0, 0, lon)
