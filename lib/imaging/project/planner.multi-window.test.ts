...
test('shouldKeepExistingDeliverableTonight drops a scheduled sub whose window already ended', () => {
  ...
  const scheduledNight: ProjectNight = {
    ...
    status: 'scheduled',
    plannedStartIso,
    // 4 × 300s + overhead ends well before now
    filterPlansTonight: [{ filterName: 'Red', exposureSeconds: 300, count: 4 }],
    ninaSequenceJson: '{"mock":true}',
  }
  const project: ImagingProject = {
  ...
  assert.equal(
    shouldKeepExistingDeliverableTonight(project, free, weatherOpen, nightKey, now),
    false
  )
})
