...
export function computeScheduleInsight(
  ...
  for (let i = 0; i < ordered.length; i += 1) {
    ...
    /* Past windows no longer occupy free time — they must be re-placed below. */
    if (pEnd <= nowMs) continue
  ...
  for (let reqIndex = 0; reqIndex < ordered.length; reqIndex += 1) {
    ...
    if (
      ...
      const existingStart = Date.parse(req.plannedStartIso!)
      const existingEnd = existingStart + estimateDurationSeconds(req) * 1000
      /* Keep only still-future (or overlapping now) commitments; past slots replan. */
      if (Number.isFinite(existingEnd) && existingEnd > nowMs) continue
    }
