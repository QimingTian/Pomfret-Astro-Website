      ...
      if (typeof fov === 'number' && fov > 0) {
        ...
        if (isMosaic && mosaicOverlayRefs?.current && mosaicPanels?.length) {
          for (const [id, el] of Array.from(mosaicOverlayRefs.current.entries())) {
            ...
            // Grid mosaic: one shared rotation at the view center so panels stay a rigid
            // overlap grid. Custom panels: parallactic rotation at each panel's sky position.
            const panelRotDeg = useSensorLayout
              ? boresightRotDeg
              : (() => {
