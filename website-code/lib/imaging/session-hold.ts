...
export async function setQueueSessionOnHold(sessionId: string): Promise<{ ok: true } | { error: string }> {
...
export async function releaseQueueSessionHold(sessionId: string): Promise<{ ok: true } | { error: string }> {
  const current = await getRequestById(sessionId)
  if (!current) return { error: 'Session not found' }
...
export async function releaseProjectNightHold(
  ...
  const restore: ProjectNightStatus =
    night.onHoldFromStatus === 'scheduled' || night.onHoldFromStatus === 'planned'
      ? night.onHoldFromStatus
      : 'planned'
