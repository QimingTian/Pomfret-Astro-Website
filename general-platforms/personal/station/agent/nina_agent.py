#!/usr/bin/env python3
"""
Pomfret Astro Personal Edition — NINA polling agent.

Set POMFRET_HUB_BASE_URL (default http://127.0.0.1:7841) via Station Agent UI or env.
...
def _hub_base_url() -> str:
    raw = os.environ.get("POMFRET_HUB_BASE_URL", "http://127.0.0.1:7841").strip()
    return raw.rstrip("/")


def _api_url(path: str) -> str:
    return f"{_hub_base_url()}{path}"


SEQUENCE_JSON_URL = _api_url("/api/imaging/nina-sequence")
RECONCILE_QUEUE_URL = _api_url("/api/imaging/reconcile-queue-schedule")
AGENT_PULSE_URL = _api_url("/api/imaging/agent-pulse")
ESTOP_DELIVERY_URL = _api_url("/api/imaging/emergency-stop/delivery")
AGENT_EVENTS_URL = _api_url("/api/imaging/agent-events")
