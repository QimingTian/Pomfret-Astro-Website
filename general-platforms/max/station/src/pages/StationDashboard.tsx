...
import { normalizeProductPlan, planDisplayLabel } from '../lib/plan-label'
...
export function StationDashboard() {
  ...
  useEffect(() => {
    void (async () => {
      const [c, t] = await Promise.all([loadConfig(), loadTenant()])
      const nextConfig =
        normalizeProductPlan(t.plan) === 'max' && !c.siteDisplayName.trim() && t.displayName.trim()
          ? { ...c, siteDisplayName: t.displayName.trim() }
          : c
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setSaveMsg(null)
    try {
      if (normalizeProductPlan(tenant?.plan) === 'max') {
        const siteName = config.siteDisplayName.trim()
        if (!siteName) {
          setSaveMsg('Site name is required for FRAOS Max.')
          return
        }
        await syncSiteDisplayName(siteName)
      }
  ...
  const isMaxPlan = normalizeProductPlan(tenant?.plan) === 'max'
  ...
  return (
    <div className="client-shell">
      <StationHeader
        edition={planDisplayLabel(tenant?.plan)}
        running={running}
        busy={busy}
          ...
          <div className="station-right-stack">
            ...
            <section className="station-glass-pane">
              ...
              <form className="settings-form" onSubmit={(e) => void handleSave(e)}>
                {isMaxPlan ? (
                  <label>
                    <span>Site name (required)</span>
                    <input
                      type="text"
                      value={config.siteDisplayName}
                      onChange={(e) => setConfig({ ...config, siteDisplayName: e.target.value })}
                      placeholder="e.g. Backyard observatory"
                      required
                    />
                    <p className="settings-hint">Shown in Control when you queue sessions to this site.</p>
                  </label>
                ) : null}
