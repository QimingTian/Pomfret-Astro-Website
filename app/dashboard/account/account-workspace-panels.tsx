...
export function AccountWorkspacePanels({ isAdmin }: { isAdmin: boolean }) {
  const sections = useMemo(() => accountSectionsForUser(isAdmin), [isAdmin])
  const [active, setActive] = useState<AccountWorkspaceSectionId>(() =>
    defaultAccountSection(isAdmin)
  )
  ...
  if (isAdmin) {
    return (
      <>
        ...
        <AdminWorkspaceSection active={active} />
      </>
    )
  }
...
function AdminWorkspaceSection({ active }: { active: AccountWorkspaceSectionId }) {
  ...
  if (active === 'observatory') {
    const siteName = resolveObservatorySite(t.siteId).name
    return (
      <DashboardPanel title="Observatory Status">
        <p className="mb-3 text-xs text-gray-400">Site: {siteName}</p>
  ...
  if (active === 'log') {
    return (
      ...
      >
        <AdminActivityLogPanel entries={t.logEntries} loading={t.logLoading} error={t.logError} />
      </DashboardPanel>
    )
  }
  ...
  if (active === 'my-sessions') {
    return <AccountMemberGrid />
  }

  if (active === 'gallery') {
    return (
      <AccountTwoColRow
        left={<GalleryRequestsSection />}
        right={<GallerySubmissionSection />}
      />
    )
  }

  if (active === 'imaging') {
    return <ImagingRequestsSection />
  }
  ...
  if (active === 'members') {
    return <AllMembersSection />
  }

  return null
}

/** @deprecated Use AccountWorkspacePanels — kept for imports during transition. */
export function AdminDashboardGrid() {
  return <AccountWorkspacePanels isAdmin />
}
