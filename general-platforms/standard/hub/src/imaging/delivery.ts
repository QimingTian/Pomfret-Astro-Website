...
> {
  ...
  if (isEmergencyStopStopping()) {
    const { getEmergencyStopState } = await import('../personal-estop.js')
    ...
    if (queueId && !state?.deliveredAt && markEmergencyStopDelivered(queueId)) {
      return { kind: 'json', status: 200, body: estopSequenceJson(tenantId ?? 'local', queueId) }
    }
  }
  ...
  if (queueId) {
    ...
    if (projectNight) {
      if (lower.includes('completed') || lower.includes('sequence finished')) {
        ...
        emitLiveEvent(liveProgressChannel(projectNight.projectId), { type: 'status', queueStatus: 'completed' })
        void reconcilePendingScheduleStatus()
      } else if (lower.includes('failed') || lower.includes('error')) {
        markNightFailed(projectNight.id)
        emitLiveEvent(liveProgressChannel(projectNight.projectId), { type: 'status', queueStatus: 'failed' })
      } else {
        emitLiveEvent(liveProgressChannel(projectNight.projectId), { type: 'progress', text })
      }
      return { ok: true, queueId }
    }
    if (lower.includes('completed') || lower.includes('sequence finished')) {
      patchSessionStatus(queueId, 'completed')
      emitLiveEvent(liveProgressChannel(queueId), { type: 'status', queueStatus: 'completed' })
    } else if (lower.includes('failed') || lower.includes('error')) {
      patchSessionStatus(queueId, 'failed')
      emitLiveEvent(liveProgressChannel(queueId), { type: 'status', queueStatus: 'failed' })
    } else {
      emitLiveEvent(liveProgressChannel(queueId), { type: 'progress', text })
    }
  }
