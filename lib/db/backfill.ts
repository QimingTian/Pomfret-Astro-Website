...
import { imagingEquipmentKvKey } from '@/lib/imaging/equipment/equipment-store'
import type { MemberUser } from '@/lib/member-store'
import { OBSERVATORY_SITES } from '@/lib/observatory-sites'
import { scopedKvKey, withObservatorySiteAsync } from '@/lib/observatory-site-scope'
...
export async function backfillPostgresFromKv(): Promise<{ ok: true; skipped?: string } | { ok: false; error: string }> {
  ...
  for (const site of OBSERVATORY_SITES) {
    await withObservatorySiteAsync(site.id, async () => {
      const queueDoc = await kvPresent<{ requests?: Array<Record<string, unknown>> }>(
        scopedKvKey(REDIS_LIVE_KEYS.queue)
      )
      if (queueDoc) {
      ...
      const projectsDoc = await kvPresent<{ projects?: Array<Record<string, unknown>> }>(
        scopedKvKey(REDIS_LIVE_KEYS.projects)
      )
      if (projectsDoc) {
      ...
      const boardDoc = await kvPresent<{ entries?: Array<Record<string, unknown>> }>(
        scopedKvKey(REDIS_LIVE_KEYS.board)
      )
      if (boardDoc) {
      ...
      const auditDoc = await kvPresent<{ entries?: Array<Record<string, unknown>> }>(
        scopedKvKey(REDIS_LIVE_KEYS.audit)
      )
      if (auditDoc) {
      ...
      const equipment = await kvPresent<{ rigs?: unknown }>(imagingEquipmentKvKey())
      if (equipment) await mirrorImagingEquipment(equipment.rigs ?? equipment)

      const windowsDoc = await kvPresent<{ windows?: Array<Record<string, unknown>> }>(
        scopedKvKey(REDIS_LIVE_KEYS.closedWindows)
      )
      if (windowsDoc) {
      ...
      const objectsDoc = await kvPresent<{ byQueueId?: Record<string, string> }>(
        scopedKvKey('imaging-r2-object-map')
      )
      ...
      const previewsDoc = await kvPresent<{ byQueueId?: Record<string, string> }>(
        scopedKvKey('imaging-r2-preview-map')
      )
