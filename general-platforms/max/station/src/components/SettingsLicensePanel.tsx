...
import type { PersonalTenantInfo } from '../lib/types'
import { getLocalLicenseStatus, type LocalLicenseStatus } from '../lib/station-api'
import { fetchLicenseSummary, type LicenseSummaryResponse } from '../lib/hub-license'
import {
  formatLicenseDate,
...
export function SettingsLicensePanel({ tenant }: SettingsLicensePanelProps) {
  ...
  const refresh = useCallback(async () => {
    setLoading(true)
    try {
      const local: LocalLicenseStatus = await getLocalLicenseStatus()
  ...
  if (!view) {
    return (
      <div className="settings-license-panel">
        {loading ? <p className="settings-muted">Loading license…</p> : null}
      </div>
    )
  }
