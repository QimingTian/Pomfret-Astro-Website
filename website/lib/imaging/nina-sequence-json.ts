...
export function buildNinaSequenceJson(params: NinaSequenceParams): string {
  ...
  applyNinaHttpAuthCredentials(root, params.sessionProgressAuthPassword)
...
/** Remove Discord Alert instructions/triggers from imported templates. */
function stripDiscordNodes(node: unknown): void {
  const walk = (value: unknown): void => {
    if (!value || typeof value !== 'object') return
    if (Array.isArray(value)) {
      for (const item of value) walk(item)
    ...
    if (Array.isArray(rec['$values'])) {
      rec['$values'] = (rec['$values'] as unknown[]).filter((item) => {
        if (!item || typeof item !== 'object') return true
        const type = (item as Record<string, unknown>)['$type']
        return !(typeof type === 'string' && type.includes('DiscordAlert'))
      })
      for (const item of rec['$values'] as unknown[]) walk(item)
    }
    for (const child of Object.values(rec)) walk(child)
  }
  walk(node)
...
/** Replace template HttpAuth* fields with tenant secret (NINA Basic auth on session-progress). */
function applyNinaHttpAuthCredentials(node: unknown, password?: string): void {
  const pass = (password ?? '').trim() || process.env.NINA_SESSION_PROGRESS_BASIC_PASSWORD?.trim()
  if (!pass) return
