...
export type SiteSubmitDecision =
  | { ok: true; as: 'system_admin' | 'site_admin' | 'site_member' | 'guest_direct' | 'guest_approved' }
  | { ok: false; error: string; code: 'not_affiliated' | 'guest_closed' | 'guest_pending' | 'guest_rejected' | 'imaging_rejected' | 'imaging_pending' }
...
export function canSubmitImagingAtSite(input: {
  ...
  // Guest path — no affiliation.
  switch (input.guestAccessMode) {
    case 'closed':
      return {
        ok: false,
        error: 'This observatory is not open to guests. Request affiliation from an observatory administrator.',
        code: 'guest_closed',
      }
    case 'open_direct':
