...
import type { ImagingRequestStatus } from '@/lib/imaging-queue-store'
import type { ProjectNight } from '@/lib/imaging-project-store'
...
export function queueStatusToAuditStatus(status: ImagingRequestStatus): SessionAuditStatus {
  return status
}
...
function sessionLabel(subject: SessionAuditSubject): string {
  const name = subject.nightIndex != null ? `${subject.target} Session ${subject.nightIndex}` : subject.target
...
export async function logSessionStatusChange(input: {
...
export async function logSessionImagingPlanChanged(input: {
  subject: SessionAuditSubject
  previousPlannedStartIso?: string | null
  plannedStartIso?: string | null
  ...
  await appendAuditLog({
    kind: 'session.imaging_plan_changed',
    message: `Session imaging plan changed: ${label}.`,
    detail: {
      id: input.subject.id,
      target: input.subject.target,
      ...(input.subject.projectMode != null ? { projectMode: input.subject.projectMode } : {}),
